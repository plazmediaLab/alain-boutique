...
export default function ItemsSales({ item, filter }){
  ...
  return (
    <li 
      key={ item.id }
      className="grid items-center w-full shadow-container rounded-card"
      css={css`
    ...
    >
      <button 
        type="button"
        className="h-full rounded-none rounded-l-card bg-white text-p_blue-100 border-r border-background hover:text-green-500"
        css={css`
          padding: 0 .25rem;
        `}
      >
      ...
      <div 
        className="bg-white rounded-r-card p-2 text-title-item font-light w-full"
      ...
      <div className="on-toggle bg-white rounded-b-card p-3 pl-8 col-span-2 text-description text-bluegray-400">
        { item.comment ? <p className="pb-2">{ item.comment }</p> : null }
        <section
